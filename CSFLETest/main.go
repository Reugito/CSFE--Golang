...
func createEncryptedClient(dataKeyIdBase64 string) *mongo.Client {
	...
	mongoClient, err := mongo.Connect(ctx, options.Client().ApplyURI("mongodb://localhost:27017").SetAutoEncryptionOptions(autoEncryptionOpts))
	if err != nil {
		log.Fatal(err)
	}
	return mongoClient
...
func main() {
	...
	if _, err := collection.InsertOne(context.TODO(), bson.M{"name": "Rao", "ssn": "123456"}); err != nil {
		log.Fatal(err)
	}
